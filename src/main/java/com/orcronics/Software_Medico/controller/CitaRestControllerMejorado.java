...
public class CitaRestControllerMejorado {
	...
	public ResponseEntity<Cita> crear(@RequestBody Cita cita, @PathVariable String inicio, @PathVariable String termino,
		...
		try {
			...
			case "DISPONIBLE":System.out.println("save as disponible");
				ci = saveAsDisponibleValidador(cita, inicio, termino, pacienteid);
				break;
	...
	public Cita saveAsDisponibleValidador(Cita cita, String inicio, String termino, int pacienteid) {
		try {
			Date ahora = new Date(); System.out.println(cita); System.out.println("paciente id:"+pacienteid);
			if (pacienteid==0) { System.out.println("paciente null");
				cita.setCerrado(false);
			...
			} else { System.out.println("paciente no null");
				cita.setCerrado(false);
				cita.setEstatus("PENDIENTE");
				cita.setSolicitopodologo(cita.isSolicitopodologo());
				cita.setPaciente( pacienteService.buscar(pacienteid) );
				cita.setProcedimiento(cita.getProcedimiento());
				cita.setFechareg(ahora);
				cita.setUsuarioreg(0);
				cita.setInicio( fullFormatEn.parse(inicio) );
				cita.setTermino( fullFormatEn.parse(termino) );
	...
	public Cita saveAsPendiente(Cita cita, String inicio, String termino, int pacienteid) {
		try {
			Date ahora = new Date();
			//verifico si la cita no tiene ID, entonces lo registro como disponible.
			if ( cita.getId()==0 )
				cita = saveAsDisponible(cita, inicio, termino);
	...
	public ResponseEntity<?> obtenerPorLocalYFechaYEspecialidad(@PathVariable int local, @PathVariable String fecha, 
		...
		try {
			...
			if ( especialidad.equals("OTROS"))
				list = citaService.buscarTodosByLocalYFechaYEspecialidadOtros(local, dateFormat.parse(fecha));
			else
				list = citaService.buscarTodosByLocalYFechaYEspecialidad(local, dateFormat.parse(fecha), especialidad);
